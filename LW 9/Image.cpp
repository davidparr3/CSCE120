...
//
Image::Image(const Image& picture) : image(nullptr), width(picture.width), height(picture.height) {
     allocateImage(width, height);
     for(unsigned int y = 0; y < height; y++){
        for(unsigned int x = 0; x < width; x++){
            image[x][y] = picture.image[x][y];
        }
     }
}
...
Image& Image::operator=(const Image& picture) {
    if (this != &picture) {
      clear();
      width = picture.width;
