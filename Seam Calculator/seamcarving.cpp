...
int main() {
  ...
  // TODO: add code to validate input (part 1) Exit program with error if there are problems with targetWidth or targetHeight
  if(static_cast<int>(targetWidth) < 0 || static_cast<int>(targetHeight) < 0 
    || static_cast<unsigned int>(targetWidth) > width || static_cast< unsigned int>(targetHeight) > height){
